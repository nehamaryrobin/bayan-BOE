...
INSERT INTO boe_header (
    ...
    %(DEC_DATE_HIJRI_2)s, %(DEC_DATE_GREGORIAN_2)s,
    %(DEC_TYPE_3)s, %(PORT_TYPE_4)s,
    %(DELIVERY_ORDER_NO_5)s, %(IMPORTER_EXPORTER_6)s,
    %(UNLOAD_DATE_7A)s, %(NET_WEIGHT_7B)s,
    ...
    %(PACKAGES_16)s, %(AWB_NO_17A)s, %(MANIFEST_NO_17B)s,
    %(PORT_OF_LOADING_18)s, %(MARKS_NUMBERS_19)s,
    %(PORT_OF_DISCHARGE_20)s, %(DESTINATION_21)s,
...
WHERE dec_no = %s AND pdf_filename = %s
"""


def is_duplicate(conn, dec_no: str, pdf_filename: str) -> bool:
    cursor = conn.cursor()
